# Create a raw transaction with a transaction fee of 20,000 satoshis 
# The transaction fee should be 20,000 satoshis, then the rest of the amount should be sent to the address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP
# Use the UTXOs from the transaction below
raw_tx=""
recipient="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
amount=$(bitcoin-cli -regtest decoderawtransaction $raw_tx | jq '[.vout[].value] | add * 100000000 | round')  
utxo_txid=$(bitcoin-cli -regtest decoderawtransaction $raw_tx | jq  -r ".txid") 
utxo_vout0=$(bitcoin-cli -regtest decoderawtransaction $raw_tx | jq  -r ".vout | .[0] | .n")
utxo_vout1=$(bitcoin-cli -regtest decoderawtransaction $raw_tx | jq  -r ".vout | .[1] | .n")


# Convert satoshis to BTC (to send amount need to be in btc not sat)
amount_to_send=$(jq -n "($amount - 20000) / 100000000")

rawtxhex=$(bitcoin-cli -regtest -rpcwallet=btrustwallet createrawtransaction "[{\"txid\": \"$utxo_txid\", \"vout\": $utxo_vout0},{\"txid\": \"$utxo_txid\", \"vout\": $utxo_vout1}]" "[{\"$recipient\": $amount_to_send}]")
echo $rawtxhex
