# Calculate the total output value of this transaction in satoshis
raw_tx=""
# value_btc=$(bitcoin-cli -regtest decoderawtransaction $raw_tx | jq -r ".vout | .[] | .value " | awk '{s+=$1} END {print s}')
# sat_amount=$(jq -n "$value_btc * 100000000")
sat_amount=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx" | jq '[.vout[].value] | add * 100000000 | round')

echo $sat_amount