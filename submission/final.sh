...
UTXO_VALUE=$(bitcoin-cli -regtest decoderawtransaction $BASE_TX | jq -r ".vout.[0].value * 100000000 | round")
...
# WRITE YOUR SOLUTION BELOW:
TX_SIZE=$(bitcoin-cli -regtest decoderawtransaction $BASE_TX | jq -r '.vsize')
check_cmd "Transaction size calculation" "TX_SIZE" "$TX_SIZE"

FEE_RATE=10  # satoshis/vbyte
FEE_SATS=$(jq -n "$TX_SIZE * $FEE_RATE")
...
CHANGE_AMOUNT=$(jq -n "$UTXO_VALUE - $PAYMENT_AMOUNT - $FEE_SATS")
check_cmd "Change calculation" "CHANGE_AMOUNT" "$CHANGE_AMOUNT"

# Convert amounts to BTC for createrawtransaction
PAYMENT_BTC=$(jq -n "$PAYMENT_AMOUNT / 100000000" | xargs printf "%.8f")
CHANGE_BTC=$(jq -n "$CHANGE_AMOUNT / 100000000" | xargs printf "%.8f")
...
VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq -r ".vout[] | select(.scriptPubKey.address == \"$PAYMENT_ADDRESS\") | .value" | xargs printf "%.8f")
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE=$(echo "$DECODED_TX" | jq -r ".vout[] | select(.scriptPubKey.address == \"$CHANGE_ADDRESS\") | .value" | xargs printf "%.8f")
...
CHILD_SEND_AMOUNT=$(jq -n "$CHANGE_AMOUNT - $CHILD_FEE_SATS")
check_cmd "Child amount calculation" "CHILD_SEND_AMOUNT" "$CHILD_SEND_AMOUNT"

# Convert to BTC
CHILD_SEND_BTC=$(jq -n "$CHILD_SEND_AMOUNT / 100000000")
...
SECONDARY_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$SECONDARY_TX" | jq -r '.vout[0].value * 100000000 | round')
...
TIMELOCK_AMOUNT=$(jq -n "$SECONDARY_OUTPUT_VALUE - $TIMELOCK_FEE")
check_cmd "Timelock amount calculation" "TIMELOCK_AMOUNT" "$TIMELOCK_AMOUNT"

# Convert to BTC
TIMELOCK_BTC=$(jq -n "$TIMELOCK_AMOUNT / 100000000")
